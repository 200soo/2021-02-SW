...
float dist_min, dist_max, dist_raw, dist_ema, dist_prev, alpha, duty_neu; // unit: mm
...
// adjust servo position according to the USS read value
  ...
  if(dist_raw <= 180) {
     ...
     duty_neu = _DUTY_MIN + (((_DUTY_MAX - _DUTY_MIN) / 180) * (dist_ema - 180));
     // _DUTY_MAX - _DUTY_MIN 으로 0~180 사이의 각도를 만들어내고, 180으로 나눔
     // _DUTY_MIN을 더해서 0~180 사이의 각도로 작동할 수 있도록 함
     // dist_raw - 180은 180~360 사이에 있는 물체의 위치를 0~180으로 나타냄
     // 거리가 0~180 사이의 값을 가지기 때문에 1도 당 DUTY를 곱하면 각도를 알아낼 수 있음
